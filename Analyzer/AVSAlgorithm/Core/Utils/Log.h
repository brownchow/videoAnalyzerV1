#ifndef AVSALGORITHM_LOG_H
#define AVSALGORITHM_LOG_H
#include <time.h>
#include <string>
...
#define LOGI(format, ...)  fprintf(stderr,"[AVSALGORITHM::INFO]%s [%s:%d] " format "\n", logTime().data(),__func__,__LINE__,##__VA_ARGS__)
#define LOGE(format, ...)  fprintf(stderr,"[AVSALGORITHM::ERROR]%s [%s:%d] " format "\n",logTime().data(),__func__,__LINE__,##__VA_ARGS__)

}

#endif //AVSALGORITHM_LOG_H